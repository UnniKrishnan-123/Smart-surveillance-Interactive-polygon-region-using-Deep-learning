...
rf = Roboflow(api_key="dGtNfRzB42tmUIZayf6V")
new_project = rf.workspace().create_project(
    project_name="regioncounting",
    project_license="MIT",
...
def run(
    ...
    csv_filename = 'object_detection_results.csv'
    csv_file = open(csv_filename, 'w', newline='')
    csv_writer = csv.writer(csv_file)

# Write the header row to the CSV file
    csv_writer.writerow(['Timestamp','Frame Number', 'Object ID', 'Class', 'X', 'Y', 'Width', 'Height'])
    ...
    output_width = 1280  # Set your desired width
    output_height = 720  # Set your desired height
    video_writer = cv2.VideoWriter(str(save_dir / f'{Path(source).stem}.mp4'), fourcc, fps, (output_width, output_height))
...
# Resize the frame to the desired width and height
    ...
    while videocapture.isOpened():
        if not pause:
            ...
            objects_inside_region = []  # Store objects inside the region

        for box, track_id, cls in zip(boxes, track_ids, clss):
            x, y, w, h = box
            object_center = (x, y)

            # Check if the object is inside any of the created regions
            is_inside_any_region = any(is_inside_polygon(object_center, region['polygon']) for region in counting_regions)

            if is_inside_any_region:
                # Extract relevant information
                frame_number = vid_frame_count
                object_id = track_id
                class_label = str(names[cls])

                # Append the information to the list of objects inside the region
                objects_inside_region.append([frame_number, object_id, class_label, x, y, w, h])

        # Save the information of objects inside the region to the CSV file
        for obj_info in objects_inside_region:
    ...
    # Inside the loop where frames are processed
    
           

# Close the CSV file at the end of processing
    csv_file.close()
    del vid_frame_count
